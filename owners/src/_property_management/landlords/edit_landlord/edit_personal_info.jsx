...
const EditLandlordPersonalInfo = () => {
    ...
    useEffect(() => {
        getLandlordPersonalInfo()
    }, [token, landlordId])

    const getLandlordPersonalInfo = async () => {
        try {
            const response = await axios.get(
                `${baseUrl}/manage-landlord/create-landlord/basic-info?landlord_id=${landlordId}`,
                {
                    headers: {
            ...
            if (response.data.success) {
                ...
                setUploadedImage(data.document_url)
                if (data) {
                    setValue("name", String(data.name));
                    setValue("email", String(data.email));
                    setValue("phone", String(data.phone));
                    setValue("next_of_kin_name", String(data.next_of_kin_name));
                    setValue("next_of_kin_relationship", String(data.next_of_kin_relationship));
                    setValue("next_of_kin_phone", String(data.next_of_kin_phone));
                    setValue("id_or_passport_number", String(data.id_or_passport_number));
                }
            } else {
                toast.error(response.data.message)
            }
        } catch (error) {
            toast.error("Error fetching landlord basic info!")
        }
    }
    ...
    const onSubmit = async (values) => {
        try {
            ...
            toast.error(error.response.data.error.email || error.response.data.error.passport || error.response.data.error.phone || "Error sending data")
        }
    }
    ...
    return (
        <>
            <div className="p-4 flex justify-between mx-4">
                <div>
                    <h1 className="text-xl font-bold text-gray-700">Add new landlord</h1>
                    <p className="text-sm text-gray-500">Create new landlord and link them to a property unit </p>
                </div>
            </div>
                        ...
                        <div className="flex justify-between">
                            <div className="h-full w-full">
                                ...
                                <div className="mt-4 mb-6">
                                    <h3 className="font-semibold text-sm">Uploaded Image:</h3>
                                    <div className="flex flex-wrap space-x-3">
                                        <div className="w-16 h-16 m-1 border p-0.5 rounded">
                                            <img
                                                src={uploadedImage}
                                                alt={`Uploaded`}
...
export default EditLandlordPersonalInfo