...
const AddTenantProperty = () => {
  const [rentDueDate, setRentDueDate] = useState(new Date());
  const [reminderDate, setReminderDate] = useState(new Date());
  const [finesDate, setFinesDate] = useState(new Date());
  ...
  const schema = z.object({
    ...
    amount_criteria: z.enum(["current_full_month_rent", "current_full_month_rent_balance",
      "total_cumulative_balances_inclusive_of_previous_month"
    ], {
      errorMap: () => ({ message: "You must select either 'Current full month rent', 'Current full month rent balance', or 'Total cumulative balances inclusive of previous month'" })
    }).optional(),
    rent_due_date: z.date().transform((date) => format(date, 'dd')),
    due_rent_reminder_date: z.date().transform((date) => format(date, 'dd')),
    due_rent_fine_start_date: z.date().transform((date) => format(date, 'dd')),

    mode_for_late_payment: z.string().optional(),
    criteria_percentage: z.string().optional(),
    late_payment_fixed_amount: z.string().optional(),
  }).superRefine((data, ctx) => {
    if (data.mode_for_late_payment === "percentage") {
      if (!data.amount_criteria) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          message: "Amount criteria is required when mode is percentage",
      ...
      if (!data.criteria_percentage) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          message: "Criteria percentage is required when mode is percentage",
      ...
      if (!data.late_payment_fixed_amount) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          message: "Fixed amount is required when mode is fixed amount",
    ...
    const fetchProperties = async () => {
      ...
      try {
        const propertyResponse = await axios.get(
          "https://pm.api.rentnasi.com/api/v1/manage-tenant/required-data/available-properties",
          {
            headers: {
  ...
  const handlePropertyChange = async (event) => {
    ...
    try {
      ...
      const unitsResponse = await axios.get(
        `https://pm.api.rentnasi.com/api/v1/manage-tenant/required-data/available-units?property_id=${propertyId}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
      ...
      console.log("Getting units for Property:", error)
    }
  }
  ...
  const handleUnitChange = async (event) => {
    ...
    try {
      ...
      const unitDetailsResponse = await axios.get(`https://pm.api.rentnasi.com/api/v1/manage-tenant/required-data/selected-unit?unit_id=${unitId}`, {
        headers: {
          Authorization: `Bearer ${token}`
        }
      })
  ...
  const onSubmit = async (values) => {
    try {
      ...
      const response = await axios.post("https://pm.api.rentnasi.com/api/v1/manage-tenant/create-tenant/other-info", dataToSend,
        {
          headers: {
            Authorization: `Bearer ${token}`
          }
        ...
        <div className="bg-white rounded-xl shadow col-span-2 p-4 mx-4 h-full">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            ...
            <div className="flex space-x-6">
              <h6 className="text-sm font-medium text-gray-900">Is the rent and deposit agreed </h6>
              <label>
                <input
            ...
            <div className="flex justify-between space-x-4">
              <div className="w-full">
                ...
                <Controller
                  name="rent_due_date"
                  control={control}
                  defaultValue={rentDueDate}
                  render={({ field }) => (
                    <DatePicker
                      aria-label="rent_due_date"
                      className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded focus:ring-red-500 focus:border-red-500 block w-full p-2.5"
                      selected={field.value}
                      onChange={(date) => setRentDueDate(field.onChange(date))}
              ...
              <div className="w-full">
                ...
                <Controller
                  name="due_rent_reminder_date"
                  control={control}
                  defaultValue={reminderDate}
                  render={({ field }) => (
                    <DatePicker
                      aria-label="due_rent_reminder_date"
                      className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded focus:ring-red-500 focus:border-red-500 block w-full p-2.5"
                      selected={field.value}
                      onChange={(date) => setReminderDate(field.onChange(date))}
                      dateFormat="dd/MM/yyyy"
                      wrapperClassName="w-full"
                    />
                  )}
            ...
            <div className="flex justify-between space-x-4">
              <div className="w-full">
                ...
                <Controller
                  name="due_rent_fine_start_date"
                  control={control}
                  defaultValue={finesDate}
                  render={({ field }) => (
                    <DatePicker
                      aria-label="due_rent_fine_start_date"
                      className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded focus:ring-red-500 focus:border-red-500 block w-full p-2.5"
                      selected={field.value}
                      onChange={(date) => setFinesDate(field.onChange(date))}
                      dateFormat="dd/MM/yyyy"
                      wrapperClassName="w-full"
                    />
                  )}
