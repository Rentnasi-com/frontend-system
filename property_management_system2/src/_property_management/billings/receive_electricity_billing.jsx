...
const ReceiveElectricityBilling = () => {
    ...
    const handleFloorChange = async (event) => {
        ...
        try {
            const responseElectricityBill = await axios.get(`${baseUrl}/manage-tenant/electricity-billing?property_id=${selectedProperty}&floor_id=${floorId}`,
                {
                    headers: {
                        Authorization: `Bearer ${token}`,
                    },
            ...
            if (responseElectricityBill.data.success) {
                const billsWithEditableFields = responseElectricityBill.data.results.map(bill => ({
                    ...bill,
                ...
                setElectricityBills(billsWithEditableFields || [])
            }
        } catch (error) {
    ...
    const handleUnitPriceChange = (index, value) => {
        const updatedBills = [...electricityBills];
        updatedBills[index].editableUnitPrice = parseFloat(value) || 0;
        setElectricityBills(updatedBills);
    };
    ...
    const prepareSubmitData = (singleBill = null) => {
        ...
        const billsToSubmit = electricityBills.filter(bill =>
            electricityBills.some(b => b.isSelected) ? bill.isSelected : true
        ).filter(bill => bill.editableCurrentReading > 0 && bill.hasValidReading);

        if (billsToSubmit.length === 0) {
            const invalidBills = electricityBills.filter(bill =>
    ...
    const handleSubmit = async (singleBill = null) => {
        ...
        try {
            const response = await axios.post(
                `${baseUrl}/manage-tenant/electricity-billing`,
                submitData,
                {
                    headers: {
                        Authorization: `Bearer ${token}`,
            ...
            if (response.status == 200) {
                toast.success("Electricity readings submitted successfully!");
                                            ...
                                            <tr>
                                                <th className="px-4 py-2 w-8">
                                                    <input
                                                        type="checkbox"
                                                        onChange={(e) => {
                                                            const updatedBills = electricityBills.map(bill => ({
                                                                ...bill,
                            ...
                            {electricityBills.length > 0 && (
                                <div className="flex justify-end py-2 px-4">
                                    <button
                                        ...
                                        disabled={isSubmitting || electricityBills.length === 0}
                                        className={`px-2 py-1 rounded-md text-sm text-white ${isSubmitting || electricityBills.length === 0
                                            ? 'bg-gray-400'
...
export default ReceiveElectricityBilling
