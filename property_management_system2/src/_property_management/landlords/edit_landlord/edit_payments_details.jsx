...
const EditLandlordPaymentsDetails = () => {
    ...
    const getLandlordPaymentsDetails = async () => {
        try {
            const response = await axios.get(`${baseUrl}/manage-landlord/create-landlord/other-info?landlord_id=${landlordId}`,
                {
                    headers: {
                        Authorization: `Bearer ${token}`
                    }
    ...
    const handleSelectChange = (event) => {
        ...
        if (selectedId && !selectedProperties.includes(selectedId)) {
            setSelectedProperties(prev => [...prev, selectedId]);
        }
        event.target.value = "";
    };
    ...
    const handleRemoveProperty = (propertyId) => {
        setSelectedProperties(prev =>
            prev.filter(id => id !== Number(propertyId))
        );

        setValue("properties", selectedProperties.filter(id => id !== Number(propertyId)));
    };
    ...
    const onSubmit = async (values) => {
        try {
            ...
            const response = await axios.patch(`${baseUrl}/manage-landlord/create-landlord/other-info`, dataToSend,
                {
                    headers: {
                        Authorization: `Bearer ${token}`,
...
export default EditLandlordPaymentsDetails