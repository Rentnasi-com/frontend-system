...
const EditTenantProperty = () => {
    ...
    // Add these useEffect hooks after your existing watchers

    // Clear electricity fields when electricity meter read is disabled
    useEffect(() => {
        if (is_electricity_meter_read === "0") {
            setValue("electricity_unit_price", "");
            setValue("initial_electricity_reading", "");
        }
    }, [is_electricity_meter_read, setValue]);

    // Clear percentage fields when fixed amount mode is selected
    useEffect(() => {
        if (mode_for_late_payment === "fixed_amount") {
            setValue("amount_criteria", "");
            setValue("criteria_percentage", "");
        }
    }, [mode_for_late_payment, setValue]);

    // Clear fixed amount when percentage mode is selected
    useEffect(() => {
        if (mode_for_late_payment === "percentage") {
            setValue("late_payment_fixed_amount", "");
        }
    }, [mode_for_late_payment, setValue]);

    // Clear tax percentage when not taxable
    useEffect(() => {
        if (is_taxable === "0") {
    ...
    // Clear initial meter reading when meter read is disabled
    useEffect(() => {
        if (is_meter_read === "0") {
            setValue("initial_meter_reading", "");
        }
    }, [is_meter_read, setValue]);
