        ...
        try {
            ...
            if (images.length > 0 && images[0].data_url) {
                ...
                if (!conversionResponse.data.success) {
                    toast.error("Error while uploading your image!" || conversionResponse.data.message);
                    return;
                }
            ...
            if (response.data.success) {
                ...
                toast.error(response.data.message || "Error updating staff")
            }
        } catch (error) {
    ...
    const fetchStaffDetails = async () => {
        ...
        try {
            ...
            if (response.data.success) {
                ...
                // Set personal information from staff object
                ...
                // Set employment information from profile object
                setValue("role", profile.role || "");
                setValue("department", profile.department || "");
                setValue("salary", profile.salary || "");
                setValue("hire_date", profile.hire_date || "");
                setValue("is_manager", profile.is_manager ? 1 : 0);
                setValue("active", profile.active ? 1 : 0);

                // Set next of kin information from profile object
                ...
                setValue("emergency_contact_relationship", profile.emergency_contact_relationship || "");

                // Set image if exists
                if (staff.image) {
                    setImages([{ data_url: staff.image }]);
                }
            ...
            toast.error(error.response?.data?.message || "Error fetching staff details")
    ...
    return (
        <>
            <div className="p-4 flex justify-between mx-4">
                <div>
                    <h1 className="text-xl font-bold text-gray-700">Edit staff information</h1>
                    <p className="text-sm text-gray-500">Update staff details and employment information</p>
                </div>
            </div>
                    ...
                    <form className="space-y-4" onSubmit={handleSubmit(onSubmit)}>
                        ...
                        <div className="flex flex-row-reverse mt-4">
                            <button
                                ...
                                type="submit"
                                className="flex focus:outline-none text-white bg-red-700 hover:bg-red-800 focus:ring-4 focus:ring-red-300 font-medium rounded text-sm px-5 py-2.5 disabled:opacity-50 disabled:cursor-not-allowed">
                                {isSubmitting ? (
                                    ...
                                    <div className="flex justify-center items-center space-x-2">
                                        <p>Update & Continue</p> <FaArrowRight />
                                    </div>
                                )}
                            </button>
                        </div>
