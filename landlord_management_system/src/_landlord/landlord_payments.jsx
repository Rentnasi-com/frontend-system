...
const LandlordPayments = () => {
    ...
    const schema = z.object({
        ...
        if (data.payment_method === "mpesa") {
            if (!data.mpesa_method) return false;
            if (data.mpesa_method === "pay_bill" && !data.mpesa_pay_bill_number) return false;
            if (data.mpesa_method === "phone_number" && !data.mpesa_phone_number) return false;
            if (data.mpesa_method === "buy_goods" && !data.mpesa_till_number) return false;
        } else if (data.payment_method === "bank") {
            if (!data.bank_account_number || !data.bank_account_name || !data.bank_name || !data.bank_branch || !data.bank_code) return false;
        }

        if (data.is_property_created === 1) {
            if (!data.properties || data.properties.length === 0) return false;
        }
        ...
        const fetchProperty = async () => {
            ...
            try {
                const propertyResponse = await axios.get(
                    "https://pm.api.rentnasi.com/api/v1/manage-landlord/required-data/available-properties",
                    {
                        headers: {
    ...
    const handleSelectChange = (event) => {
        ...
        if (selectedOption && !selectedProperties.includes(selectedOption)) {
            setSelectedProperties((prev) => [...prev, selectedOption])
        }
    }
    ...
    const onSubmit = async (values) => {
        try {
            ...
            const response = await axios.post("https://pm.api.rentnasi.com/api/v1/manage-landlord/create-landlord/other-info", dataToSend,
                {
                    headers: {
                        Authorization: `Bearer ${token}`,
                            ...
                            <>
                                <div className="w-full">
                                    ...
                                    {isLoadingProperties ? (
                                        ...
                                        >
                                            <option value="" disabled>Select property to link to this landloard</option>
                                            {propertiesOptions}
                                        </select>
                                    )}
